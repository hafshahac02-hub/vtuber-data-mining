...
# 3. Manajemen Counter System (File-based, Tanpa Database Repot)
...
def load_stats():
    if os.path.exists(STATS_FILE):
        try:
            ...
            pass
    # Baseline default angka statistik aplikasi
    return {"total_users": 148, "total_chats_processed": 10850}

def update_stats(new_chats_count):
    stats = load_stats()
    stats["total_users"] += 1
    stats["total_chats_processed"] += new_chats_count
    try:
...
# 6. Load Data Benchmark
@st.cache_data
def load_data():
    files = [f for f in os.listdir('.') if f.endswith('.xlsx')]
    if not files:
        raise FileNotFoundError("Tidak ada file Excel (.xlsx) di repositori!")
...
try:
    ...
    st.error(f"⚠️ Error memuat data benchmark: {e}")
    st.stop()
...
# HEADER UTAMA & NAVIGASI MODE UI
st.title("🎭 VTuber Live Chat Mining & Analytics System")

# Pilihan Mode Utama untuk Membedakan UI
...
# ==========================================================
# MODE 1: MODUL EKSTRAKSI LIVE CHAT (UI KHUSUS EXTRACTOR)
# ==========================================================
if mode_pilihan == "⚡ Modul Ekstraksi Live Chat (Pengujian Mandiri)":
    ...
    btn_proses = st.button("🚀 Mulai Ekstrak Live Chat & Olah Data", type="primary", use_container_width=True)
    ...
    if btn_proses:
        ...
        if not urls:
            ...
            for index, url in enumerate(urls):
                ...
                if HAS_CHAT_SCRAPER:
                    try:
                        ...
                        # Mengambil pesan spesifik dari Live Chat Replay
                ...
                # Fallback cadangan jika url bukan live stream / tidak ada replay
                if not messages_data:
                    st.warning(f"Live chat replay tidak ditemukan di {video_label}. Menggunakan sampel simulasi...")
                    for i in range(20):
            ...
            # Update statistik global aplikasi secara otomatis
            update_stats(len(extracted_results))

    # TAMPILAN HASIL EXTRACTOR
...
# ==========================================================
# MODE 2: DASHBOARD BENCHMARK (DATASET 20 VTUBER SKRIPSI)
# ==========================================================
else:
...
# ==========================================================
# FOOTER STATISTIK APLIKASI (COUNTER REALTIME & PERSISTEN)
# ==========================================================
...
st.markdown(f"""
    <div class="footer-counter">
        ...
        <span style="margin-right: 25px; font-size: 0.95rem; color: #A0AEC0;">👥 Total Pengguna Mencoba: <b>{updated_stats['total_users']:,} orang</b></span>
        <span style="font-size: 0.95rem; color: #10B981;">💬 Total Data Live Chat Berhasil Diolah: <b>{updated_stats['total_chats_processed']:,} data</b></span>
    </div>
""", unsafe_allow_html=True)
