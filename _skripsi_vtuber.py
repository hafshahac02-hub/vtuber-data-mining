...
if mode_pilihan == "⚡ Modul Ekstraksi Live Chat (Realtime)":
        ...
        if not original_url:
            ...
            try:
                ...
                if not extracted_rows:
                    ...
                    st.success(f"✅ Berhasil mengekstrak dan menganalisis **{len(df_res):,} baris** live chat!")
    ...
    if 'real_extracted_data' in st.session_state and not st.session_state['real_extracted_data'].empty:
        ...
        m1.markdown(f'<div class="metric-card"><div class="metric-title">Total Live Chat Ter-ekstrak</div><div class="metric-value">{total_real:,}</div></div>', unsafe_allow_html=True)
        m2.markdown(f'<div class="metric-card"><div class="metric-title">Sentimen Positif</div><div class="metric-value" style="color:{COLOR_POS}">{pos_pct:.1f}%</div><div class="metric-sub">{pos_real:,} chat</div></div>', unsafe_allow_html=True)
        m3.markdown(f'<div class="metric-card"><div class="metric-title">Sentimen Negatif</div><div class="metric-value" style="color:{COLOR_NEG}">{neg_pct:.1f}%</div><div class="metric-sub">{neg_real:,} chat</div></div>', unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        
        # 2. GRAFIK SENTIMEN & TOPIK LDA
        c_g1, c_g2 = st.columns(2)
        with c_g1:
            st.markdown("##### Proporsi Sentimen (Naïve Bayes)")
            fig_sent = px.pie(df_real, names='Prediksi Sentimen', color='Prediksi Sentimen',
                              color_discrete_map={'Positif': COLOR_POS, 'Negatif': COLOR_NEG}, hole=0.55)
            st.plotly_chart(style_fig(fig_sent), use_container_width=True)

        with c_g2:
            st.markdown("##### Proporsi Topik LDA Dominan")
            fig_topik = px.pie(df_real, names='Topik LDA', hole=0.55, color_discrete_sequence=COLOR_THEME)
            st.plotly_chart(style_fig(fig_topik), use_container_width=True)

        # 3. GRAFIK SEBARAN SENTIMEN PER TOPIK & TOP KATA
        c_g3, c_g4 = st.columns(2)
        with c_g3:
            st.markdown("##### Sebaran Sentimen per Topik LDA")
            fig_top_sent = px.histogram(df_real, x='Topik LDA', color='Prediksi Sentimen', barmode='group',
        ...
        with c_g4:
            ...
            semua_kata = " ".join(df_real['Pesan Bersih (Sastrawi)'].dropna()).split()
            kata_counts = Counter(semua_kata).most_common(10)
            if kata_counts:
                df_words = pd.DataFrame(kata_counts, columns=['Kata', 'Frekuensi'])
                fig_words = px.bar(df_words, x='Frekuensi', y='Kata', orientation='h',
